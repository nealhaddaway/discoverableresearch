...
#'
#' Removes stopwords from a text string (adapted from litsearchr) and returns the remaining words as a vector of strings
...
#' Suggest title words
#'
#' Suggests possible title words by extracting uni-, bi-, and tri-grams from a long text (e.g. article full text), having 
#' removed punctuation and stopwords. Returns the remaining words as a vector of strings and assesses whether they are 
#' already present in the title or abstract
...
#' @param suggest A logical argument of TRUE or FALSE. If TRUE, the output dataframe returned is subsetting to only include 
#' potential keywords (i.e. those not already in the abstract or keywords)
#' @return A dataframe consisting of potential candidate title words and their suitability. If suggest = FALSE, only good 
...
#' fulltext <- readr::read_file("data/fulltext.rds")
#' fulltext <- gsub("\n", " ", fulltext)
